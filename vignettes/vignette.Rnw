...
By applying the \Rfunction{plot.profiles} method, the \Rpackage{Segvis} packages generates a \Robject{ggplot} object, thus the user can modify the output by using some their commands.
