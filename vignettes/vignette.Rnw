...
package. \Rpackage{Segvis} provides different tools to summarize and
visualize sequencing data, including but not limited to the following
tasks:

\begin{itemize}
\item Plot peaks of different conditions accross the same set of
  regions
...
For this document it is pretended to show how \Rpackage{Segvis}
interacts with some \Bioconductor{} and \R{} packages to represent the
data.
...
following classes \Robject{profile}, \Robject{profileMatrix} and
\Robject{profileMatrixList}. The first one is used to store the reads
for a given bam file, the second is the one used to interact with the
data and the third one is simply a list where all their elements are
made from \Robject{profileMatrix} objects.

\subsection{Creating a \Robject{profile} object}

To create an \Robject{profile} object, there are needed two sources of
data, the regions that are need to explore and a set of aligned
reads. To define a \Robject{profile}, it is necessary to define some
...
\Rpackage{Segvis} is going to warn the user if there aren't any
regions loaded. To load the regions a \Robject{GRanges} is necessary,
to build one there are some instructions in \ref{gr1}.
...
To create a the coverage curves it is necessary to follow three steps
\Rcode{loadReads}, \Rcode{matchReads} and \Rcode{getCoverage}:
