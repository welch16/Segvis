...
\begin{enumerate}
\item One or more bam files of aligned read data (e.g. from ChIP-seq experiments)

\item Coordinates for regions of interest

\end{enumerate}

 \Rpackage{Segvis} provides different tools to summarize and
visualize these data, including but not limited to the following tasks:

\begin{itemize}
\item Extract read data of specified input regions
\item Plot data from different files (conditions) accross the same set of regions, e.g. peak plots for ChIP-seq
